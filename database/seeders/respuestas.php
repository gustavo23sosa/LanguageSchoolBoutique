<?php

namespace Database\Seeders;

use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;

class respuestas extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        //
        DB::table('Respuestas')->insert([
            'ID'=>1,
            'respuestas'=> "Were you at the park?.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>2,
            'respuestas'=> "Is you at the park?.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>3,
            'respuestas'=> "Are you at the park?.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>4,
            'respuestas'=> "Her",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>5,
            'respuestas'=> "He",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>6,
            'respuestas' => "She",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>7,
            'respuestas'=> "It",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>8,
            'respuestas'=> "Its",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>9,
            'respuestas'=> "He",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>10,
            'respuestas'=> "the",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>11,
            'respuestas'=> "—-",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>12,
            'respuestas'=> "an",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>13,
            'respuestas'=> "the, An",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>14,
            'respuestas'=> "a, The",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>15,
            'respuestas'=> "an, The",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>16,
            'respuestas'=> "go",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>17,
            'respuestas'=> "goes",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>18,
            'respuestas'=> "going",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>19,
            'respuestas'=> "Him",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>20,
            'respuestas'=> "He",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>21,
            'respuestas'=> "His",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>22,
            'respuestas'=> "at",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>23,
            'respuestas'=> "in",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>24,
            'respuestas'=> "to",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>25,
            'respuestas'=> "go",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>26,
            'respuestas'=> "goes",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>27,
            'respuestas'=> "going",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>28,
            'respuestas'=> "I eat breakfast every morning.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>29,
            'respuestas'=> "She will watch the movie later.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>30,
            'respuestas'=> "They are studying for the test.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>31,
            'respuestas'=> "dog's",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>32,
            'respuestas'=> "dogs'",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>33,
            'respuestas'=> "dogs",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>34,
            'respuestas'=> "A",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>35,
            'respuestas'=> "An",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>36,
            'respuestas' => "The",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>37,
            'respuestas'=> "Eats",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>38,
            'respuestas'=> "Eat",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>39,
            'respuestas'=> "Eating",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>40,
            'respuestas'=> "She",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>41,
            'respuestas'=> "He",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>42,
            'respuestas'=> "They",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>43,
            'respuestas'=> "To be",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>44,
            'respuestas'=> "To do",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>45,
            'respuestas'=> "To have",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>46,
            'respuestas'=> "One daughter",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>47,
            'respuestas'=> "No children",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>48,
            'respuestas'=> "One son",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>49,
            'respuestas'=> "One son and one daughter",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>50,
            'respuestas'=> "Sarah",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>51,
            'respuestas'=> "Father",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>52,
            'respuestas'=> "Mother",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>53,
            'respuestas'=> "John",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>54,
            'respuestas'=> "Kitchen",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>55,
            'respuestas'=> "Living room",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>56,
            'respuestas'=> "Dining room",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>57,
            'respuestas'=> "Bedroom",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>58,
            'respuestas'=> "Two",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>59,
            'respuestas'=> "One",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>60,
            'respuestas'=> "Three",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>61,
            'respuestas'=> "Four",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>62,
            'respuestas'=> "Brush their teeth",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>63,
            'respuestas'=> "Play",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>64,
            'respuestas'=> "Cook",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>65,
            'respuestas'=> "Sleep",
            'activo' => 1,
        ]);



        DB::table('Respuestas')->insert([
            'ID'=>66,
            'respuestas'=> "a) He don't like pizza.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>67,
            'respuestas'=> "b) She doesn't likes pizza.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>68,
            'respuestas'=> "c) They don't like pizza.",
            'activo' => 1,
        ]);//1
        DB::table('Respuestas')->insert([
            'ID'=>69,
            'respuestas'=> "a) do",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>70,
            'respuestas'=> "b) did",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>71,
            'respuestas'=> "c) have done",
            'activo' => 1,
        ]);//2
        DB::table('Respuestas')->insert([
            'ID'=>72,
            'respuestas'=> "a) good",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>73,
            'respuestas'=> "b) well",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>74,
            'respuestas'=> "c) better",
            'activo' => 1,
        ]);//3
        DB::table('Respuestas')->insert([
            'ID'=>75,
            'respuestas'=> "a) The weather today is more nice than yesterday.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>76,
            'respuestas'=> "b) My car is older than your car.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>77,
            'respuestas'=> "c) She sings gooder than her sister.",
            'activo' => 1,
        ]);//4
        DB::table('Respuestas')->insert([
            'ID'=>78,
            'respuestas'=> "a) I have been working here since two years.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>79,
            'respuestas'=> "b) I have been working here for two years.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>80,
            'respuestas'=> "c) I have been working here in two years.",
            'activo' => 1,
        ]);//5
        DB::table('Respuestas')->insert([
            'ID'=>81,
            'respuestas'=> "a) most delicious",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>82,
            'respuestas'=> "b) deliciousiest",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>83,
            'respuestas'=> "c) deliciouser",
            'activo' => 1,
        ]);//6
        DB::table('Respuestas')->insert([
            'ID'=>84,
            'respuestas'=> "a) Funnier",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>85,
            'respuestas'=> "b) More funny",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>86,
            'respuestas' => "c) Funniest",
            'activo' => 1,
        ]);//7
        DB::table('Respuestas')->insert([
            'ID'=>87,
            'respuestas'=> "a) He doesn't likes coffee.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>88,
            'respuestas'=> "b) He doesn't like coffee.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>89,
            'respuestas'=> "c) He doesn't liking coffee.",
            'activo' => 1,
        ]);//8
        DB::table('Respuestas')->insert([
            'ID'=>90,
            'respuestas'=> "a) to",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>91,
            'respuestas'=> "b) at",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>92,
            'respuestas'=> "c) in",
            'activo' => 1,
        ]);//9
        DB::table('Respuestas')->insert([
            'ID'=>93,
            'respuestas'=> "a) at",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>94,
            'respuestas'=> "b) on",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>95,
            'respuestas'=> "c) with",
            'activo' => 1,
        ]);//10
        DB::table('Respuestas')->insert([
            'ID'=>96,
            'respuestas'=> "a) to find",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>97,
            'respuestas'=> "b) finding",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>98,
            'respuestas'=> "c) find",
            'activo' => 1,
        ]);//11
        DB::table('Respuestas')->insert([
            'ID'=>99,
            'respuestas'=> "a) to study",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>100,
            'respuestas'=> "b) studying",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>101,
            'respuestas'=> "c) study",
            'activo' => 1,
        ]);//12
        DB::table('Respuestas')->insert([
            'ID'=>102,
            'respuestas'=> "a) lost",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>103,
            'respuestas'=> "b) lose",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>104,
            'respuestas'=> "c) losted",
            'activo' => 1,
        ]);//13
        DB::table('Respuestas')->insert([
            'ID'=>105,
            'respuestas'=> "a) Did",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>106,
            'respuestas'=> "b) Do",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>107,
            'respuestas'=> "c) Does",
            'activo' => 1,
        ]);//14
        DB::table('Respuestas')->insert([
            'ID'=>108,
            'respuestas'=> "a) more beautiful",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>109,
            'respuestas'=> "b) beautifuler",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>110,
            'respuestas'=> "c) beautifuller",
            'activo' => 1,
        ]);//15
        DB::table('Respuestas')->insert([
            'ID'=>111,
            'respuestas'=> "a) It is the most ecologically diverse natural park.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>112,
            'respuestas'=> "b) It is the largest U.S. national park.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>113,
            'respuestas'=> "c) It is the country’s first national park.",
            'activo' => 1,
        ]);//16
        DB::table('Respuestas')->insert([
            'ID'=>114,
            'respuestas'=> "a) Last summer",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>115,
            'respuestas'=> "b) Last autumn",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>116,
            'respuestas'=> "c) Last winter",
            'activo' => 1,
        ]);//17
        DB::table('Respuestas')->insert([
            'ID'=>117,
            'respuestas'=> "a) At a campground",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>118,
            'respuestas'=> "b) At a hotel",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>119,
            'respuestas'=> "c) In a rental home",
            'activo' => 1,
        ]);//18
        DB::table('Respuestas')->insert([
            'ID'=>120,
            'respuestas'=> "a) Gray wolves",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>121,
            'respuestas'=> "b) Wild horses",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>122,
            'respuestas'=> "c) Bald eagles",
            'activo' => 1,
        ]);//19
        DB::table('Respuestas')->insert([
            'ID'=>123,
            'respuestas'=> "a) The geyser took about 40 minutes to erupt.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>124,
            'respuestas'=> "b) They were still wet from swimming in the Boiling River.",
            'activo' => 1,
        ]);
        DB::table('Respuestas')->insert([
            'ID'=>125,
            'respuestas'=> "c) They arrived an hour before the geyser opened to the public.",
            'activo' => 1,
        ]);//20


        DB::table('Respuestas')->insert([ 
            'ID'=>126, 
            'respuestas'=> "a) They're going to the store.", 
            'activo' => 1, 
        ]);
        DB::table('Respuestas')->insert([ 
            'ID'=>127, 
            'respuestas'=> "b) Their going to the store.", 
            'activo' => 1, 
        ]);
        DB::table('Respuestas')->insert([ 
            'ID'=>128, 
            'respuestas'=> "c) There going to the store.", 
            'activo' => 1, 
        ]);
        DB::table('Respuestas')->insert([ 'ID'=>129, 'respuestas'=> "d) Theres going to the store.", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>130, 'respuestas'=> "a) I love listening to music.", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>131, 'respuestas'=> "b) Running is my favorite form of exercise.", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>132, 'respuestas'=> "c) After finishing his homework, he went to bed.", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>133, 'respuestas'=> "a) at", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>134, 'respuestas'=> "b) to", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>135, 'respuestas'=> "c) for", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>136, 'respuestas'=> "a) Active", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>137, 'respuestas'=> "b) Still", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>138, 'respuestas'=> "c) Calm", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>139, 'respuestas'=> "a) Look out", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>140, 'respuestas'=> "b) Look up", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>141, 'respuestas'=> "c) Look through", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>142, 'respuestas'=> "a) Give up", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>143, 'respuestas'=> "b) Take up", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>144, 'respuestas'=> "c) Carry on", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>145, 'respuestas'=> "a) Admiration", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>146, 'respuestas'=> "b) Revulsion", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>147, 'respuestas'=> "c) Repulsion", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>148, 'respuestas'=> "a) Resent", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>149, 'respuestas'=> "b) Loathe", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>150, 'respuestas'=> "c) Savor", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>151, 'respuestas'=> "a) Himself", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>152, 'respuestas'=> "b) Herself", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>153, 'respuestas'=> "c) All of the above", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>154, 'respuestas'=> "a) Which", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>155, 'respuestas'=> "b) These", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>156, 'respuestas'=> "c) Those", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>157, 'respuestas'=> "a) using too many exclamation points", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>158, 'respuestas'=> "b) using run-on sentences", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>159, 'respuestas'=> "c) using too many semicolons", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>160, 'respuestas'=> "a) using 'effect' instead of 'affect'", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>161, 'respuestas'=> "b) using 'your' instead of ' you're '", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>162, 'respuestas'=> "c) using 'imply' instead of 'infer'", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>163, 'respuestas'=> "My bike has been stolen!", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>164, 'respuestas'=> "The newspaper is received everyday.", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>165, 'respuestas'=> "The cookies have been eaten by the children.", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>166, 'respuestas'=> "a) Questions about the effects of environmental problems", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>167, 'respuestas'=> "b) Two opinions about environmental problems", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>168, 'respuestas'=> "c) All of the factors that damage the environment", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>169, 'respuestas'=> "a) Air pollution", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>170, 'respuestas'=> "b) Climate change", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>171, 'respuestas'=> "c) Deforestation", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>172, 'respuestas'=> "a) Excessive carbon dioxide emissions", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>173, 'respuestas'=> "b) Uncontrollable forest fires", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>174, 'respuestas'=> "c) Smog formation in valleys", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>175, 'respuestas'=> "a) Deforestation", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>176, 'respuestas'=> "b)Droughts", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>177, 'respuestas'=> "c) Flooding", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>178, 'respuestas'=> "a) Top Environmental Concerns", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>179, 'respuestas'=> "b) Protecting Endangered Species", 'activo' => 1, ]);
        DB::table('Respuestas')->insert([ 'ID'=>180, 'respuestas'=> "c) Water Conservation Strategies", 'activo' => 1, ]);


    }
}
